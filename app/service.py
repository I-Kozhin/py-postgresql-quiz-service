...
def get_one_question_from_api() -> QuestionDtoInput:
...
def get_unique_questions(number: int) -> List[QuestionDtoOutput]:
    ...
    while len(unique_questions) <= number:
        try:
            question_external = get_one_question_from_api()
            # print(question_external)
            question_dto = create_question(question_external)
            if question_dto:
                continue
            if not question_dto:
                print('Error till saving in DB')
                continue
            unique_questions.append(question_dto)
